...
int main (void)
{
	...
	for (;;) {
		xputc('>');
		get_line((BYTE*)Line, sizeof(Line));
		ptr = (char*)Line;

		switch (*ptr++) {

        case 'z' :
            xprintf(PSTR("Opretter fil fil...\n"));
            f_open(&file1, "0:nyfil.txt", (BYTE)4);
            
            f_close(&file1);

            f_open(&file1, "0:nyfil.txt", (BYTE)3);
            xprintf(PSTR("Fil åbnet...\n"));
            for (i=0;i<1000;i++){
                if (i%10 == 0){
                    xprintf(PSTR("%d\n"),i);
                }
                f_write(&file1, "Hej med dig---", 15, &s2);
            }
